...
func NewManager(cfg *config.Config) (*Manager, error) {
	...
	if cfg.Proxy.HTTPClient.Cache.Enabled != nil && *cfg.Proxy.HTTPClient.Cache.Enabled {
		if cfg.Proxy.HTTPClient.Cache.Path == nil {
			return nil, fmt.Errorf("proxy.http_client.cache.path is required when cache is enabled")
		}
		st, err := httpclient.NewStore(*cfg.Proxy.HTTPClient.Cache.Path)
		if err != nil {
			return nil, err
		}
		m.cacheStore = st
	...
	for _, pl := range cfg.Playlists {
		for _, ch := range pl.Channels {
			po := mergePlayouts(cfg.Playout, pl.Playout, ch.Playout)
			idsByStateDir[po.StateDir] = append(idsByStateDir[po.StateDir], hashid.New(pl.Name, ch.Name))
		}
	}
...
func (m *Manager) createClient(clientConf config.Client) (*Client, error) {
	...
	cl, err := NewClient(clientConf, urlGen, m.config.ChannelRules, m.config.PlaylistRules, m.publicURLBase, m.cacheStore)
	if err != nil {
...
func (m *Manager) addPlaylistProvider(cl *Client, playlistConf config.Playlist) error {
	...
	for _, channelConf := range playlistConf.Channels {
		po := mergePlayouts(m.config.Playout, playlistConf.Playout, channelConf.Playout)
		gen := m.channelGenerator(playlistConf.Name, channelConf, po)
		if err := cl.BuildChannelProvider(playlistConf, channelConf, po, m.config.Proxy, gen, pl); err != nil {
			return fmt.Errorf(
...
func (m *Manager) channelGenerator(parentPlaylist string, channelConf config.Channel, po config.Playout) *channelgen.Channel {
